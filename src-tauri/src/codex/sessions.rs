...
pub async fn get_sessions(offset: u32, limit: u32) -> Result<SessionsResponse, String> {
...
pub async fn get_session_content(id: String) -> Result<Vec<SessionEntry>, String> {
    let home = dirs::home_dir().ok_or("Cannot find home directory")?;
    let codex_home = home.join(".codex");
            ...
            "response_item" => {
                ...
                match sub {
                    ...
                    "file" => {
                        ...
                        let content_s = payload.and_then(|p| p.get("content")).and_then(|t| t.as_str()).unwrap_or("").to_string();
                        out.push(SessionEntry::FileEntry { timestamp, path: path_s, content: content_s });
                    },
