export const en = {
  // Sidebar
  "dashboard": "Dashboard",
  "configuration": "Configuration",
  "about": "About",
  "collapse": "Collapse",
  "expand": "Expand",

  // Header
  "home": "Home",
  "theme.dark": "Switch to light",
  "theme.light": "Switch to dark",

  // Home - Hero
  "hero.eyebrow": "Local multi-protocol bridge",
  "hero.lead": "Point Codex at {url} and let {name} handle the wire protocol translation. Start the bridge below to expose {c}, {m} and {r} from one local endpoint.",
  "hero.configure": "Configure Provider",
  "hero.logs": "View Live Logs",
  "hero.default_port": "default port",
  "hero.bridge_version": "bridge version",

  // Home - Codex Connection
  "codex.title": "Codex Connection",
  "codex.live": "Live",
  "codex.idle": "Idle",
  "codex.status": "Status",
  "codex.base_url": "Base URL",
  "codex.catalog": "Catalog",
  "codex.provider": "Provider",
  "codex.manage": "Manage in Configuration →",
  "codex.copy": "Copy Codex config.toml snippet",
  "codex.copied": "Copied!",

  // BridgeStatus
  "bridge.start": "Start Bridge",
  "bridge.stop": "Stop Bridge",
  "bridge.running": "Running",
  "bridge.stopped": "Stopped",
  "bridge.port": "Port",
  "bridge.pid": "PID",
  "bridge.uptime": "Uptime",
  "bridge.online": "Bridge Online",
  "bridge.offline": "Bridge Offline",
  "bridge.starting": "Starting",
  "bridge.serving": "Serving requests",
  "bridge.booting": "Booting up...",
  "bridge.copy": "Copy",
  "bridge.url": "URL",
  "bridge.idle": "Click Start to sync to Codex",

  // Config
  "config.settings": "Settings",
  "config.title": "Configuration",
  "config.providers": "Providers",
  "config.providers.desc": "Add as many as you like. The active one is what the bridge uses.",
  "config.providers.remove_title": "Remove this provider?",
  "config.providers.ok": "Yes",
  "config.providers.cancel": "No",
  "config.providers.remove_confirm": "Remove {name}?",
  "config.providers.placeholder": "LLM model provider",
  "config.providers.add": "Add",
  "config.providers.add_title": "Add Model Provider",
  "config.providers.active": "Codex Active:",
  "config.wire_api": "Wire API",
  "config.wire_api.desc": "Choose the protocol. Model and Base URL are filled separately below.",
  "config.wire_api.tooltip": "Which API protocol this upstream provider speaks",
  "config.form.title": "Settings for Provider",
  "config.form.reset": "Reset",
  "config.form.empty": "Add a provider above to get started.",
  "config.form.test": "Test connection",
  "config.form.test_latency": "Latency",
  "config.form.test_need_url": "Please enter Base URL first.",
  "config.form.test_error": "Test failed",
  "config.form.model": "Model",
  "config.form.model_placeholder": "Select or type a model",
  "config.form.fetch_models": "Fetch models",
  "config.form.fetch_models_loading": "Fetching...",
  "config.form.fetch_models_success": "Fetched {count} models",
  "config.form.fetch_models_failed": "Failed to fetch models",
  "config.form.fetch_models_empty": "No models found",
  "config.form.model_select_placeholder": "Type or select a model",
  "config.form.model_type": "Input Modalities",
  "config.form.model_type_text": "Text",
  "config.form.model_type_image": "Image",
  "config.form.wire_api": "Wire API",
  "config.form.base_url": "Base URL",
  "config.form.base_url_placeholder": "https://api.example.com/v1",
  "config.form.api_key": "API Key",
  "config.form.api_key_placeholder": "Your API key",
  "config.form.api_key_header": "API Key Header",
  "config.form.api_key_header_tooltip": "The HTTP header name for the API key, e.g. X-Api-Key or Authorization",
  "config.form.api_key_header_placeholder": "X-Api-Key",
  "config.wire.anthropic": "Anthropic (/v1/messages)",
  "config.wire.chat_completions": "Chat Completions (/v1/chat)",
  "config.wire.chat": "Chat Completions (/v1/chat)",
  "config.wire.openai": "OpenAI Responses (/responses)",
  "config.tab.general": "General",
  "config.tab.connection": "Connection",

  "config.default_model": "Default Model",
  "config.default_model_placeholder": "Select default model",
  "config.sync_all": "Sync to Codex",
  "config.models": "Models",
  "config.models_count": "{count} models",
  "config.model_name_placeholder": "Model name",
  "config.context": "Context",
  "config.compact": "Compact",
  "config.modalities": "Modalities",
  "config.modality_text": "text",
  "config.modality_image": "image",
  "config.test_model_placeholder": "Select model",
  "config.test": "Test",
  "config.saved": "Config saved",
  "config.synced_all": "Synced all providers to Codex",
  "config.save_failed": "Failed to save",
  "config.add_model": "Add Model",
  "config.no_models": "No models yet. Click the fetch button to get model list.",
  "config.fetch_models": "Fetch models",

  "config.general.title": "General",
  "config.general.desc": "App launch behavior and config file location.",
  "config.autostart.title": "Launch at login",
  "config.autostart.desc": "Start evocode automatically when you log in (off by default).",
  "config.autostart.enabled": "Launch at login enabled",
  "config.autostart.disabled": "Launch at login disabled",
  "config.autostart.error": "Failed to update launch at login",
  "config.configdir.title": "Config file directory",
  "config.configdir.open": "Open config directory",
  "config.configdir.opened": "Opened directory",
  "config.configdir.error": "Failed to open config directory",
  "config.bridge_port.title": "Bridge Port",
  "config.bridge_port.desc": "Bridge listen port (1024-65535). Restart the bridge for changes to take effect.",
  "config.bridge_port.saved": "Port saved. Please restart the bridge",
  "config.bridge_port.error": "Failed to save port",
  "config.bridge_port.invalid": "Port must be between 1024 and 65535",
  "config.bridge_port.running_warn": "Bridge is running. Please stop it before changing the port",
  "config.max_body_size.title": "Max Request Body Size",
  "config.max_body_size.desc": "Maximum request body size in MB (default 1024). 0 = unlimited. Restart bridge to apply.",
  "config.max_body_size.saved": "Max body size saved. Please restart the bridge",
  "config.max_body_size.error": "Failed to save max body size",
  "config.max_body_size.unit": "MB",
  "config.limits.auto_compact_tip": "Auto-compact triggers when the context exceeds this limit, preserving recent conversation.",
  "config.limits.auto_compact": "Auto compact limit",
  "config.limits.title": "Context & auto-compact",
  "config.limits.desc": "Drag the sliders or click the preset values.",
  "config.limits.context_window_tip": "Maximum context window the model supports. Drag or click a preset to adjust.",
  "config.limits.context_window": "Context window",
  "config.limits.context": "Context window",
  "config.limits.compact": "Auto compact limit",
  "config.limits.tokens": "tokens",
  "config.limits.of": "of",
  "config.limits.preset_small": "Small",
  "config.limits.preset_standard": "Standard",
  "config.limits.preset_large": "Large",
  "config.limits.preset_huge": "Huge",
  "config.limits.preset_128k": "Small",
  "config.limits.preset_256k": "Standard",
  "config.limits.preset_512k": "Large",
  "config.limits.preset_1m": "1M",
  "config.limits.preset_10m": "10M",
  "config.save": "Save Config",
  "config.sync": "Sync to Codex",
  "config.sync.placeholder": "Sync to Codex...",
  "config.sync.select": "Sync",
  "config.sync.no_providers": "No options",
  "config.sync.done": "Synced",
  "config.sync.failed": "Sync failed",

  // Logs
  "logs.title": "Bridge Logs",
  "logs.empty_title": "No logs yet",
  "logs.empty_sub": "Bridge is {status}.",
  "logs.running": "running",
  "logs.stopped": "stopped",
  "logs.refresh": "Refresh",
  "logs.clear": "Clear view",
  "logs.auto_scroll": "Auto-scroll",
  "logs.label_starting": "Starting",
  "logs.label_running": "Running",
  "logs.label_error": "Error",
  "logs.label_stopped": "Stopped",

  // Update
  "update.check": "Check Update",
  "update.checking_title": "Checking for Updates",
  "update.up_to_date": "Already up to date!",
  "update.found": "Update Available",
  "update.error": "Update check failed",
  "update.modal_version": "New version",
  "update.modal_current": "Current version:",
  "update.download": "Download",
  "update.modal_skip": "Maybe later",


  // About page
  "about.title": "About evocode",
  "about.desc": "evocode is a local multi-protocol bridge that lets Codex CLI call Anthropic, OpenAI, and Chat Completions-compatible AI providers through a single unified endpoint.",
  "about.features": "Core Features",
  "about.feat_messages": "Anthropic Messages API (/v1/messages)",
  "about.feat_chat": "Chat Completions API (/v1/chat/completions)",
  "about.feat_responses": "OpenAI Responses API (/responses)",
  "about.feat_unified": "Single local endpoint, no config switching needed",
  "about.tech": "Technical Info",
  "about.version": "Version",
  "about.port": "Port",
  "about.team": "Team",
  "about.team_desc": "evocode is developed and maintained by the rbatis ORM team.",
  "about.team_what": "rbatis is a high-performance, safe and feature-rich Rust ORM framework, well received in the open-source community.",
  "about.contact": "Contact Us",
  "about.email": "Email",
  "about.email_val": "zhuxiujia@qq.com",

  // Session
  "session.title": "Sessions",
  "session.detail": "Session Detail",
  "session.back": "Back",
  "session.model": "Model",
  "session.tokens": "Tokens",
  "session.entries": "Entries",
  "session.context_tokens": "Context",
  "session.total_tokens": "Total Tokens",
  "session.expand": "Expand All",
  "session.collapse": "Collapse All",
  "session.empty": "No readable content",

  // Prompts
  "prompts.title": "Prompts",
  "prompts.presets": "Presets",
  "prompts.new": "New",
  "prompts.no_presets": "No presets yet",
  "prompts.no_presets_hint": "Click \"New\" to create your first prompt",
  "prompts.select_hint": "Select or create a prompt",
  "prompts.select_hint_desc": "Choose a preset from the left list, or click \"New\" to create one",
  "prompts.name_placeholder": "Prompt name",
  "prompts.content": "Content",
  "prompts.content_placeholder": "Write your prompt content (Markdown)",
  "prompts.chars": "chars",
  "prompts.save": "Save",
  "prompts.activate": "Apply to Codex",
  "prompts.agents_exists": "Saved",
  "prompts.agents_missing": "Not saved",
  "prompts.new_preset_name": "New Prompt",
  "prompts.delete_title": "Delete Prompt",
  "prompts.delete_ok": "Delete",
  "prompts.delete_cancel": "Cancel",
    "prompts.activated_ok": "Applied to Codex",
  "prompts.activated_fail": "Sync failed, check console logs",
  "prompts.saved_ok": "Prompt saved",
  "prompts.save_fail": "Save failed",
  "prompts.delete_confirm": "Delete preset \"{name}\"?",
}


